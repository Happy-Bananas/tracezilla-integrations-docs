...
Complete the [TypeScript installation and configuration](../typescript.html) first. Both location arguments are required, keeping the source-to-target relationship visible.
...
|---|---|
| CLI arguments, safety, and output | `src/cli/synchronize-inventory.ts` |
| Matching, mapping, and decisions | `src/workflows/synchronize-inventory.ts` |
| Shopify pagination and guarded writes | `src/shopify/shopify-inventory-service.ts` |
| tracezilla warehouse inventory | `src/tracezilla/tracezilla-inventory-service.ts` |
